...
$(document).ready(function() { // Une fois que le document (base.html.twig) HTML/CSS a bien été complètement chargé...
    ...
    console.log('ici')
    // add-collection-widget.js : fonction permettant d'ajouter un nouveau bloc "programme" au sein d'une produit (pour agrandir la collection)
    $('.add-another-collection-widget').click(function (e) {
        ...
        // Au final, l'attribut ressemblera à "produit[programmes][n°]"
        newWidget = newWidget.replace(/__name__/g, counter)
        // Ajout également des attributs personnalisés "class" et "value", qui n'apparaissent pas dans le prototype original 
        newWidget = newWidget.replace(/><input type="hidden"/, ' class="borders"><input type="hidden" value="'+produit+'"')
    ...
    // anonymize-collection-widget.js : fonction permettant de supprimer un bloc "programme" existant au sein d'une produit
    ...
    // remove-produit.js : fonction permettant de demander la confirmation de suppression d'une produit
    $('.remove-produit-confirm').on('click', function(e) {
        e.preventDefault()

        let id=$(this).data('id')
        let href=$(this).attr('href')

        showModalConfirm(id, href, "Confirmation de suppression d'une produit")
    })
